...
export var speed = 220   # how fast the player will move (pixels per sec)
...
func _on_Player_body_entered(body):
	# sunlight disappears and points earned after being hit
	if body.is_in_group("suns"):
		emit_signal("collect", body)
	elif body.is_in_group("flys"):
